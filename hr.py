...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if flooding_process is not None:
        await update.message.reply_text(' 2 Minute ruk ja madarchod .')
        return

    if flooding_command is None:
        await update.message.reply_text('dubara command run karega lawde. Use /bgmi to set parameters.')
        return
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users or datetime.datetime.now() > datetime.datetime.strptime(users[user_id], '%Y-%m-%d %H:%M:%S'):
        await update.message.reply_text("❌ Abe Babli Gand Kye le Admin se.")
        return
    ...
    flooding_process = None
    await update.message.reply_text('Ma Chud Gayi Server Ki.')
